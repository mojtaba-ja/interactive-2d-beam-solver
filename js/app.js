...
(function () {
  ...
  function emptyModel(unitsId) {
    ...
    return {
      ...
      opts: {
        ...
        snap: true, snapStep: u.defaults.snap * u.len.f, forceN: false
      }
    };
  }
  ...
  function buildSelection() {
    ...
    if (!state.selection) {
      elh('p', { class: 'hint', text: 'Click an object in the drawing or in a list above to edit it. Drag it along the beam to move it; hold Alt while dragging to ignore the snap grid.' }, p);
      return;
    }
  ...
  function showHelp() {
    modal('How to use the beam solver', function (b) {
      b.innerHTML =
        ...
        '<li>Add supports and loads from the sidebar, then <b>drag</b> them along the beam. Distributed loads have round handles at each end for the extents <code>a</code> and <code>b</code>. Hold <b>Alt</b> to bypass the snap grid.</li>' +
